...
//Use training data to train and build a decision tree
//val maxDepth =10// up to 30
...
val trainingError = labelAndPredsTraining.filter(r => r._1 == r._2).count.toDouble / data.count
val testingError = labelAndPredsTesting.filter(r => r._1 == r._2).count.toDouble / data.count
...
print(model)
//Show us the decison tree model. May be very long, depending on the depth of the tree
//println("Decision tree model is as follows:\n" + model.toDebugString)
